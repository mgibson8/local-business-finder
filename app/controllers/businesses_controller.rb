class BusinessesController < ApplicationController
  ...
  def list
    ...
    @places   = @client.spots(@location.lat, @location.lng, radius: @radius_in_metres, detail: true)


    respond_to do |format|
      format.js
      format.csv { send_data gen_csv(@places), filename: "companies-#{Date.today}.csv", type: "text/csv"}
    end
  end
